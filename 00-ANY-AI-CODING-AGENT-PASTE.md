...
## 0) AI coding agent runtime alignment — do not guess
...
Do not hardcode a model name. The operator may be using Opus 4.8 Max, Opus alias, Sonnet, or another account/provider default. Adapt to the active AI coding agent runtime.
...
```txt
Use the strongest available planning mode, then cheaper execution mode when safe.
```
...
### Phase G — AI coding agent integration within limits

Create AI coding agent project assets only after Phase A proves compatibility:
