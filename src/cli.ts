...
import {
  ...
  KERNEL_DIRECTORY_PATHS,
  KERNEL_INITIALIZED_FILE_PATHS,
  KERNEL_PROTECTED_FILE_PATHS,
  KERNEL_REQUIRED_EMPTY_LEDGER_FILES,
  beginCommand,
  beginRun,
  bootstrapKernel,
  dbStatus,
...
import { LEDGER_APPEND_CONTRACT, LEDGER_FILES } from "./lib/ledger.ts";
...
import { inspectMasterWorkflow } from "./lib/master-workflow.ts";
...
function handleVerify(ctx) {
  ...
  const checks = [
    ...
    { name: "Master Workflow infrastructure acceptance", ok: masterWorkflow.infrastructure_acceptance === "PASS", evidence: "pala workflow:check" },
  ...
  const readinessCheckNames = new Set(["Active model and effort observed", "No unresolved sync state", "No unresolved push-readiness blockers"]);
  const readinessFailures = checks.filter((check) => readinessCheckNames.has(check.name) && !check.ok);
  const failures = checks.filter((check) => !readinessCheckNames.has(check.name) && !check.ok);
  const rootBlockers = failures.flatMap((failure) => blockerIdsForVerificationFailure(failure.name, pushReadiness.blockers))
    .filter((blocker, index, all) => all.indexOf(blocker) === index);
  const assessment = failures.length === 0
    ? { decision: "pass_allowed", riskLevel: "low", requiredApproval: false, reason: "Master Workflow infrastructure checks have evidence with no unresolved infrastructure blockers." }
    : { decision: "manual_verification_required", riskLevel: "medium", requiredApproval: false, reason: `${failures.length} infrastructure verification checks have unresolved blockers.` };
  const decision = recordDecision(ctx.db, {
    runId: ctx.runId,
    decisionType: "final-verify",
    inputs: { failures: failures.map((failure) => failure.name), readiness_failures: readinessFailures.map((failure) => failure.name), release_blockers: releaseBlockers },
    assessment
  }, {
    skipLedgerAppend: true
  });
  return {
    status: failures.length === 0 ? "safe_to_execute" : "manual_verification_required",
    decision,
    ...
    readiness_blockers: releaseBlockers,
    infrastructure_acceptance: failures.length === 0 ? "PASS" : "PARTIAL",
...
async function handle(args, ctx) {
  ...
  if (name === "workflow:check") {
    return inspectMasterWorkflow();
  }
